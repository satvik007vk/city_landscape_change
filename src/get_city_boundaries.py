...
def get_city_boundary(
    city: City, time="2026-06-01T00:00:00Z", save_file: bool = False
) -> gpd.GeoDataFrame:
    ...
    city_boundary["admin_level"] = city_boundary["tags"].apply(
        lambda t: t.get("admin_level")
    )

    if save_file:
        city_boundary.to_file(
            filename=(ROOT_DIR / f"data/{city.name}_boundary.geojson"), driver="GEOJSON"
        )
